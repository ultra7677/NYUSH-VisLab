...
vg.HexGrid.prototype = {
	...
	generateTile: function(cell, scale, material, data) {
	...
	generateTiles: function(config) {
		...
		for (i in this.cells) {
			c = this.cells[i];
			if (j < settings.binList.length )
				t = this.generateTile(c, settings.tileScale, settings.material, settings.binList[j]);
			else t = this.generateTile(c, settings.tileScale, settings.material, null);
